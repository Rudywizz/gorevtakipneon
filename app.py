...
# Güvenli anahtar – Render/Prod'da ENV’den ver
app.secret_key = os.getenv("SECRET_KEY", "gizli_anahtar")

# Neon/Postgres bağlantını ENV’den oku
# Örn: postgresql+psycopg2://USER:PASS@ep-xxx-pooler.neon.tech:5432/DB?sslmode=require
DB_URI = os.getenv("SQLALCHEMY_DATABASE_URI")

# Yerelde ENV yoksa, fallback olarak sqlite kullan (opsiyonel)
if not DB_URI:
    BASE_DIR = os.path.abspath(os.path.dirname(__file__))
    DB_URI = f"sqlite:///{os.path.join(BASE_DIR, 'gorev_takip.db')}"

app.config["SQLALCHEMY_DATABASE_URI"] = DB_URI
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False

# Neon TLS ister; sadece Postgres'te sslmode=required gönder
...
class User(db.Model):
    ...
    # İleride rol eklemek istersek: role = db.Column(db.String(20), default="personel", nullable=False)
...
class Task(db.Model):
    ...
    date = db.Column(db.String(50))            # basit tutuyoruz; istersen Date yaparız
    ...
    # Şablon uyumluluğu için isim kısayolları
    @property
...
def dashboard():
    ...
    # İstersen admin'e tüm kayıtlar, normal kullanıcıya kendi kayıtları:
    # user = User.query.get(user_id)
    # if user.role == 'admin':
    #     tasks = Task.query.order_by(Task.id.desc()).all()
    # else:
    #     tasks = Task.query.filter(or_(Task.user_id == user_id, Task.assigned_to == user_id)).order_by(Task.id.desc()).all()
    tasks = Task.query.order_by(Task.id.desc()).all()
...
def export_pdf():
    ...
    pdf_io = BytesIO()
    pisa.CreatePDF(rendered, dest=pdf_io)   # xhtml2pdf string kabul eder
    pdf_io.seek(0)
...
if __name__ == '__main__':
    # Geliştirme için yerelde çalıştır
    app.run(debug=True)
